...
const getObjektMintingsDuringPeriod = async (min_time, max_time) => {
  let reply = redisClient.get(`getObjektMintingsDuringPeriod_${min_time}_to_${max_time}`)
  if (reply != null) {
    return reply
  } 
...
const getArtisticUniverse = async (max_time, min_time, limit, skip_swaps=true) => {
  ...
  let reply = await getAsync(`getArtisticUniverse_${min_time}_to_${max_time}_limit_${limit}`)
  console.log(reply)
  if (reply != null) {
  ...
  await setAsync(`getArtisticUniverse_${min_time}_to_${max_time}_limit_${limit}`, JSON.stringify(universe), 'EX', 10 * 60)
  return universe
}
...
const getFeaturedArtisticUniverse = async (max_time) => {
  ...
  artisticUniverse = await getArtisticUniverse(max_time, _subtractDays(max_time, 3), 2000)
  ...
  return artisticUniverse.filter(o =>
      ...
      ((hdaoMap[o.minter] || 0) + ((tzktscores[o.minter] || 0) * (thresholdHdao))) / 
        ...
        // Weekly activity - bot filter
          ...
          ((tzktscores[o.minter] || 0) * backendConf.filterWeeklyFreeObjkts),
          //No Less than zero
          0)  >
      thresholdHdao)
}
