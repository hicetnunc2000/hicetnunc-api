...
const getArtisticUniverse = async (max_time, min_time, limit) => {
  max_time = ((typeof max_time !== 'undefined') && max_time != 0) ? max_time : _getTimeQuantizedToMinute()
  min_time = ((typeof min_time !== 'undefined') && min_time != 0) ? min_time : _subtractWeek(max_time)
...
const getFeaturedArtisticUniverse = async (max_time) => {
  ...
  const five_days = _subtractDays(max_time, 5)
  ...
  artisticUniverse = await getArtisticUniverse(max_time, five_days, 2000)
...
const getObjectById = async (objectId, with_swaps=true, min_creation_level, max_creation_level) => {
  ...
  if (typeof min_creation_level !== 'undefined') {
    if (typeof max_creation_level !== 'undefined') {
      objectQuery = conseiljs.ConseilQueryBuilder.addPredicate(
        objectQuery,
        'block_level',
        conseiljs.ConseilOperator.BETWEEN,
