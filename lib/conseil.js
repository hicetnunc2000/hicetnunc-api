...
const _subtractTwoHours = (timestamp) => {
  return timestamp - (ONE_HOUR_MILLIS * 2)
}
...
const getFeaturedArtisticUniverse = async (max_time) => {
  ...
  const twoh_ago = _subtractTwoHours(max_time)
  mintsPerCreatorWeek = await getObjektMintingsDuringPeriod(min_time, max_time)
  mintsPerCreator2h = await getObjektMintingsDuringPeriod(twoh_ago, max_time)
  ...
  await Promise.all(
    ...
    const mscore = await axios.get(
        ...
        let score = 0
        if(data) {
          if(data.data.alias) score += 1
          if(data.data.twitter) score += 2
          if(data.data.instagram) score += 2
          if(data.data.github) score += 3
          if(data.data.site) score += 0.5
          if(data.data.reddit) score += 1
          if(data.data.email) score += 1
          if(data.data.logo) score += 1
          if(data.data.description) score += 1
        }
  ...
  return artisticUniverse.filter(o =>
      (hdaoMap[o.minter] || 100_000) / 
        Math.max(
        ((Math.max(mintsPerCreatorWeek[o.minter] || 1, 1) + 
        (2 ** Math.max((mintsPerCreator2h[o.minter] || 1) - 1, 0))) -
        (tzktscores[o.minter] || 0)), 0)  >
      thresholdHdao)
}
