...
Promise.timeout = function(t, val) {
...
Promise.raceAll = function(promises, timeoutTime, timeoutVal) {
  return Promise.all(promises.map(p => {
      return Promise.race([p, Promise.timeout(timeoutTime, timeoutVal)])
  }));
}

const queue = new Queue({
  rules: {
    cfipfs: { // Rule for sending private message via telegram API
      rate: 50,            // one message
      limit: 1,           // per second
      priority: 1
    },
...
async function getIpfsHash(ipfsHash) {
  let reply = await getAsync(ipfsHash)
  if (reply != null) {
    return JSON.parse(reply)
  }
  let data = await (axios.get('https://cloudflare-ipfs.com/ipfs/' + ipfsHash, {timeout: 1000}).then(res => res.data))
  await setAsync(ipfsHash, JSON.stringify(data), 'EX', 24*60*60)
  return data
...
async function getObjektByIdBCD(id) {
  let reply = await getAsync(`getObjektByIdBCD_${id}`)
  if (reply != null) {
    return JSON.parse(reply)
  }
  ...
  await setAsync(`getObjektByIdBCD_${id}`, JSON.stringify(objkt), 'EX', 24*60*60)
  return objkt
...
async function getObjktById(id, with_swaps=true) {
  if (!with_swaps) {
    let reply = await getAsync(`getObjktById_${id}`)
    if (reply != null) {
      return JSON.parse(reply)
    }
  }
  ...
  try {
    ...
    if (!with_swaps) {
      await setAsync(`getObjktById_${id}`, JSON.stringify(objkt), 'EX', 24*60*60)
    }
    return objkt
  }
  catch {
